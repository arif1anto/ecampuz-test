...
function kolom($table)
    {
 $query = "SELECT column_name FROM INFORMATION_SCHEMA.COLUMNS WHERE `TABLE_SCHEMA` = '$this->database' AND TABLE_NAME='$table'";

        $stmt = $this->sql->prepare($query) OR die("Error code :" . $this->sql->errno . " (bukan kolom)");

        $stmt->bind_param('s', $this->database);
        $stmt->bind_result($nama_kolom);
        $stmt->execute();
        while ($stmt->fetch()) {
            $fields[] = array('nama_kolom' => $nama_kolom);
        }
        return $fields;
        $stmt->close();
        $this->sql->close();
...
function primary_field($table)
    {
        $query = "SELECT COLUMN_NAME,COLUMN_KEY FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA='$this->database' AND TABLE_NAME='$table' AND COLUMN_KEY='PRI'";
    ...
    {
        $query = "SELECT COLUMN_NAME,COLUMN_KEY,DATA_TYPE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA='$this->database' AND TABLE_NAME='$table' AND COLUMN_KEY <> 'PRI'";
        $stmt = $this->sql->prepare($query) OR die("Error code :" . $this->sql->errno . " (not_primary_field)");
        $stmt->bind_param('ss', $this->database, $table);
    ...
    {
        $query = "SELECT COLUMN_NAME,COLUMN_KEY,DATA_TYPE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA='$this->database' AND TABLE_NAME='$table'";
        $stmt = $this->sql->prepare($query) OR die("Error code :" . $this->sql->errno . " (not_primary_field)");
        $stmt->bind_param('ss', $this->database, $table);
        $stmt->bind_result($column_name, $column_key, $data_type);
        $stmt->execute();
